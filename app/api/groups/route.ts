...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (action === "join") {
      ...
      // For public groups, check invite code
      if (group.isPublic && inviteCode) {
        if (group.inviteCode !== inviteCode) {
          return Response.json({ error: "Invalid invite code" }, { status: 400 })
        }
      } else if (!group.isPublic) {
        return Response.json({ error: "This is a private group. Contact an admin to be added." }, { status: 403 })
      }
    ...
    if (action === "leave") {
      ...
      const memberIndex = group.members.findIndex((m: any) => m.id === userId)
      if (memberIndex === -1) {
    ...
    if (action === "add_member") {
      const { targetUserId, targetUserName, targetUserRole, adminId } = await request.json()
      
      // Check if requester is admin
      if (!isUserAdmin(adminId, group)) {
